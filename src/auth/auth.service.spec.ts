...
describe('AuthService', () => {
  ...
  describe('login', () => {
    const loginDto = { email: 'admin@example.com', password: 'correct-password' };
    const hashedPassword = '$2b$12$hashedpasswordstring';
    const adminUser = { id: '1', email: 'admin@example.com', password: hashedPassword };
    ...
    it('returns an accessToken on valid credentials', async () => {
      ...
      expect(mockJwtService.sign).toHaveBeenCalledWith({ sub: '1', email: 'admin@example.com' });
    ...
    it('uses the same error message for wrong password', async () => {
      ...
      await expect(service.login({ email: 'admin@example.com', password: 'wrong' })).rejects.toThrow('Invalid credentials');
    });
  });
});
